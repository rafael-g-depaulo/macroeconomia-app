...
import { tableBgGreen, tableBorderGreen } from '@styles'
...
export const TableRow = ({
  row,
  Cell = BorderedTableCell,
}: {
  row: any
  Cell: typeof BorderedTableCell
}) => {
  return (
  ...
  return (
    <HeaderRow>
      {columns.map((column, i) => (
        <BorderedTableCell align={i === 0 ? 'inherit' : 'right'} key={column}>
...
export const Table = ({
  ...
  const _onChangePage = useCallback(
    (e: MouseEvent<HTMLButtonElement, MouseEvent> | null, newPage: any) => {
      setPage(newPage)
        ...
        <MuiTable>
          ...
          <MuiTableBody>
            {data
              ...
              .map((row: any) => (
                <RowComponent key={row.id} row={row} Cell={undefined as any} />
              ))}
            ...
            <MuiTableRow>
              <MuiTablePagination
                ...
                onChangePage={_onChangePage as any}
              />
            </MuiTableRow>
          </MuiTableBody>
        </MuiTable>
